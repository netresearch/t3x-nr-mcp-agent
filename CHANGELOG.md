...
- **An answer that claims a change the run did not make says "Nothing was saved in this step"** (NEXT-167, ADR-017). On the demo the assistant answered "Erledigt" four times in one conversation, with page and element uids of its own invention, in runs that called no tool. The run records every write the DataHandler performed as a `tool_write` step; when a completed answer reads like a change (German or English participles such as *erledigt*, *gespeichert*, *created*, *updated*) and the segment that produced it holds no such step, the message is stored with the notice `nothingSaved`, and both chat surfaces render it as a label in the reader's language. The text only triggers, the run decides. The system prompt gains the rule behind it — claim a change only on a successful write result, never state a uid no tool returned — and how to read nr-llm's `approval_denied (decided_by: run_owner)` result: the user declined it, not "the system".
- **A configuration failure is phrased for the reader** (NEXT-167, ADR-017). "API key identifier is required for provider OpenAI" was shown to everyone. The conversation now stores the kind of a failure in the new column `error_code` — `providerNotConfigured` for nr-llm's provider configuration and authentication exceptions, `chatNotConfigured` for a missing Task, configuration or model — and the request that shows it decides the wording: an administrator gets the technical message and a button to the LLM providers or tasks module, everyone else "The AI provider is not set up. Please contact your administrator." (English and German). Run the database analyzer after upgrading; existing rows have an empty code and are shown as before.
- **The assistant knows which tools exist but are not available to the user** (NEXT-167, ADR-017). With nr-llm's `UnavailableToolsResolverInterface` (nr-llm ADR-201) the system prompt lists the tools the run is not offered, one line each with the reason in plain words, so the assistant can say "there is a tool for that, it is not enabled for you — ask an administrator" instead of claiming nothing can do it. The service is looked up by name and feature-detected: with an nr-llm that does not have it, the list is empty and nothing else changes.
- **"weiter" while an approval is pending starts no second run** (NEXT-167, ADR-017). On the demo "weiter", "alles approved" and "habe alles freigegeben" each abandoned the pending approval and started a new run, which drafted the same page again. A message that consists only of such a phrase now asks the run first: still waiting — the chat answers that the decision is taken on the card (in AI Tasks for a reader who gets no card) and writes nothing; decided in AI Tasks and finished — the message is stored with a note naming the records the run wrote, and the conversation is idle; still running — the usual busy answer. The phrase is never taken as the approval itself. A longer message is an ordinary request, as before.
