...
- **"weiter" while an approval is pending starts no second run** (NEXT-167, ADR-017). On the demo "weiter", "alles approved" and "habe alles freigegeben" each abandoned the pending approval and started a new run, which drafted the same page again. A message that consists only of such a phrase now asks the run first: still waiting — the chat answers that the decision is taken on the card and writes nothing; decided in AI Tasks and finished — the message is stored with a note naming the records the run wrote, and the conversation is idle; still running — the usual busy answer. The phrase is never taken as the approval itself. A longer message is an ordinary request, as before.
