...
- **Instructions per conversation** (NEXT-172). The sliders button opens the conversation's own instructions (`tx_nrmcpagent_conversation.system_prompt`, which existed but nothing set). They replace the configured Configuration/Task prompts for that conversation; identity, language rules and the user context stay. Refused while a turn is running, because the worker writes the whole row when it settles and would put the old value back; longer than 10,000 characters is refused rather than cut.
