...
- **A configuration failure is phrased for the reader** (NEXT-167, ADR-017). "API key identifier is required for provider OpenAI" was shown to everyone. The conversation now stores the kind of a failure in the new column `error_code`, on a turn and on an approval's continuation alike — `providerNotConfigured` for nr-llm's provider configuration and authentication exceptions, `chatNotConfigured` for a missing Task, configuration or model — and the request that shows it decides the wording: an administrator gets the technical message and a button to the LLM providers or tasks module, everyone else "The AI provider is not set up. Please contact your administrator." (English and German). Run the database analyzer after upgrading; existing rows have an empty code and are shown as before.

- **The assistant knows which tools exist but are not available to the user** (NEXT-167, ADR-017). With nr-llm's `UnavailableToolsResolverInterface` (nr-llm ADR-201) the system prompt lists the tools the run is not offered, one line each with the reason in plain words, so the assistant can say "there is a tool for that, it is not enabled for you — ask an administrator" instead of claiming nothing can do it. The service is looked up by name and feature-detected: with an nr-llm that does not have it, the list is empty and nothing else changes. Remote (MCP) tools appear only for administrators.

- **"weiter" while an approval is pending starts no second run** (NEXT-167, ADR-017). On the demo "weiter", "alles approved" and "habe alles freigegeben" each abandoned the pending approval and started a new run, which drafted the same page again. A message that consists only of such a phrase now asks the run first: still waiting — the chat answers that the decision is taken on the card (in AI Tasks for a reader who gets no card) and writes nothing; decided in AI Tasks and finished — the message is stored with a line naming the records the run wrote (language-neutral for the model; the reader sees a translated label), and the conversation is idle; still running — the usual busy answer. The phrase is never taken as the approval itself. A longer message is an ordinary request, as before.

- **An nr-llm Task per backend group** (NEXT-172, ADR-015). `groupTaskMapping` takes `groupUid:taskUid` pairs; the first pair whose group the user belongs to (subgroups included) decides, everyone else keeps `llmTaskUid`. The status endpoint, the toolbar and the turn itself all resolve through `ExtensionConfiguration::getLlmTaskUid()`, which now answers for the current backend user. A Task chooses the model and the prompts; the user's permissions are still nr-llm's per-user tool policy. Before a turn runs, the Task's Configuration must be active and — if it is restricted to backend groups — available to the user (nr-llm's `ConfigurationResolver::actorMayUse()`); otherwise the turn fails with a message that says which. This applies to `llmTaskUid` as well: the agent runtime does not check it, so a Configuration withheld from a group used to run for it anyway.
...
- **A sent message can be edited and run again** (NEXT-172). *Edit* below a user message opens it in place; *Save and run again* (`POST /ai-chat/conversations/edit`) replaces it, drops every message after it and starts a new turn under the same guards and the same compare-and-swap as sending. An attachment of the edited message stays attached. The request carries the message and the transcript length the client showed; if the transcript has changed since (another tab), the edit is refused with `409` and the view reloads instead of replacing the wrong message. Editing the first message renames a conversation whose title was taken from it; a title the user chose stays.

- **Instructions per conversation** (NEXT-172). The sliders button opens the conversation's own instructions (`tx_nrmcpagent_conversation.system_prompt`, which existed but nothing set). They are added as the last block of the system prompt, between `<user_instructions>` markers and ranked below everything else in it, so the administrator's Configuration/Task prompts, the identity and the language rules stay in force. (Until now the column, had anything set it, would have replaced those prompts.) The column is written only by its own endpoint, never by a full-row save, so a turn settling later cannot put an old value back. Saving is refused while a turn is running, because that turn already runs with the previous instructions; longer than 10,000 characters is refused rather than cut.

- **An activity list of the running turn** (NEXT-172). The chat showed a typing indicator and nothing else until the answer arrived. `RunActivityRecorder` is now the `onStep` callback of `AgentRuntime::run()` and `approve()` and writes a summary of every model round and tool call — kind, round, duration, tool name, failed or not; never arguments or results — into the new column `activity`, column-only so it cannot race the turn's final write. The poll carries it on both the full and the fast path, so the list is live. A decision taken on the approval card is recorded where it happened, and the pending approval is shown as the last entry. For screen readers only the newest step is announced, through a small status region; the list itself is not a live region. A failed activity write is logged and dropped, because nr-llm calls the callback before it records the step. The ticket assumed the chat already received the run's events; it did not — it learned the run's uuid only when `run()` returned, which is too late for a live view, hence the callback. Run the database analyzer after updating.

- **An "AI Chat" dashboard widget** (NEXT-172). With EXT:dashboard installed (it stays optional: `suggest`, registered from `Configuration/Services.php` only when the dashboard's classes exist), the widget lists the user's five most recent chats and opens one — or a new one — in the floating panel next to the dashboard, falling back to the chat module. Availability follows the same check as the toolbar button. Dates are formatted in the backend user's language (`IntlDateFormatter`), titles are escaped.
...
- **`netresearch/nr-llm` is accepted at `^0.34 || ^0.35 || ^0.36`.** nr-llm 0.36.0 adds the writers this release's chat drives (NEXT-160); on a 0.x `^0.35` does not admit it, so without the widened range every installation would stay on nr-llm 0.35.
